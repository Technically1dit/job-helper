...
async def upload_resume(file: UploadFile = File(...), user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if parsed.skills: profile.skills = parsed.skills
    if parsed.experience: profile.experience = parsed.experience
    if parsed.education: profile.education = parsed.education
    if parsed.years_of_experience: profile.years_of_experience = parsed.years_of_experience
